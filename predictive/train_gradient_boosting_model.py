...
def _encode_categorical(rows: List[dict]) -> Dict[str, LabelEncoder]:
    ...
    for feature in CATEGORICAL_FEATURES:
        ...
        values = [str(row.get(feature) or "None") for row in rows]
        encoder.fit(values)
...
def _build_matrix(rows: List[dict], target_column: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    for row in rows:
        feature_row = [int(row.get(feature, 0)) for feature in FEATURE_ORDER]
...
def train_and_export(
    ...
    if unique_classes.size < 2:
        raise ValueError(
            "Training target needs at least two classes (0 and 1). "
            "Current dataset contains one class only; generate more labeled history first."
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and export a gradient boosting model for SDMS predictive service."
    )
    ...
    parser.add_argument(
        "--output-dir",
        default="artifacts/gbm_v1",
        help="Output directory for model artifacts.",
    )
    parser.add_argument(
        "--model-name",
        default="gradient_boosting_model",
